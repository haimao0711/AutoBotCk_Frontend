...
export const compactBalance = (balance: string) => {
	return +balance > 0 ? balance?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.') + ' VND' : '0';
};

export const compactAmount = (balance: string) => {
	return +balance > 0 ? Number(balance).toFixed(2).toLocaleString() : '0';
};
